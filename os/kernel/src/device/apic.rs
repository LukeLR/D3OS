...
impl Apic {
            ...
            InterruptModel::Apic(apic_desc) => {
                ...
                for irq_override in apic_desc.interrupt_source_overrides.iter() {
                    info!(
                        ...
                        irq_override.trigger_mode
                    );
                    irq_overrides.push(InterruptSourceOverride {
                ...
                for (i, io_apic_desc) in apic_desc.io_apics.iter().enumerate() {
                    ...
                    for i in io_apic_desc.global_system_interrupt_base..max_entry {
                        ...
                        unsafe {
                            io_apic.set_table_entry(i as u8, entry);
                        }
                    }
            ...
            match io_apic_for_target(&io_apics, nmi.global_system_interrupt) {
                Some(io_apic) => unsafe {
                    io_apic
                        .0
                        .lock()
                        .set_table_entry(nmi.global_system_interrupt as u8, entry);
                },
                None => warn!(
                    "No responsible IO APIC found for NMI [{}]",
    ...
    fn create_local_apic(madt: &Madt) -> LocalApic {
        ...
        process.kernelmode_address_space.map_pfr_for_vma(
            &vma,
            PhysFrameRange {
    ...
    pub fn allow(&self, vector: InterruptVector) {
        let target = target_gsi(
            &self.irq_overrides,
            vector as u8 - InterruptVector::Pit as u8,
        );
